...
| `files/` | 编译时覆盖到固件的自定义文件，结构对应固件根目录 |
| `files/etc/` | 固件 `/etc` 目录下的自定义配置文件 |
| `files/www/` | 固件 `/www` 目录下的自定义 Web 文件 |
| `img/` | README 文档中使用的教程截图 |
| `scripts/` | 编译过程中执行的辅助脚本 |
| `scripts/show_hardware_info.sh` | 显示硬件信息的脚本，编译时用于输出设备硬件参数 |
| `工具与脚本/` | 刷机相关工具和辅助脚本合集 |
| `工具与脚本/gen_config.py` | 生成设备编译配置文件的 Python 脚本 |
| `工具与脚本/主机模式host` | 切换设备为主机模式的配置或脚本 |
| `刷机脚本/` | Windows 下一键刷机所需的工具和脚本 |
| `刷机脚本/一键升级补丁.bat` | Windows 批处理脚本，自动完成固件刷入流程 |
| `diy-part1.sh` | 编译第一阶段自定义脚本，在拉取源码后执行（添加软件源、打补丁等） |
| `diy-part2.sh` | 编译第二阶段自定义脚本，在生成默认配置后执行（修改配置、添加文件等） |
| `upstream_history.txt` | 上游源码的更新历史记录 |
| `upstream_lock.txt` | 锁定上游源码版本的记录文件，用于保证编译可复现 |
